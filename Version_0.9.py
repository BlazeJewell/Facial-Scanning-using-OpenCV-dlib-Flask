...
def head_movement():
	nodDirection = " "
	noseState = (int) (FRAME_WIDTH / 2)
	xArc = (int) (FRAME_WIDTH / 10)
	noseR = (int) (xArc * 4)
	noseL = (int) (xArc * 6)
	...
	"""# move comment/uncomment to draw line grid
	xArc = (int) (FRAME_WIDTH / 10)
	xVal = xArc
	for x in range(10):
		cv2.line(frame, (xVal,0), (xVal, 1000), (0,0,255), 1)
		xVal += xArc
	#"""

	#""" draw the direction of head nod
	cv2.putText(frame, "Head nod: {}".format(nodDirection), (10, 270),
	cv2.FONT_HERSHEY_TRIPLEX, 0.7, (0, 204, 0), 2)
	#"""
	#""" draw the nose state - for testing
	cv2.putText(frame, "Nose state: {}".format(noseState), (10, 300),
	cv2.FONT_HERSHEY_TRIPLEX, 0.7, (0, 204, 0), 2)
	#"""
...
# Use this for pre recorded video
...
while True:
	...
	for rect in rects:
		...
		#draw the total number of blinks on the frame
		cv2.putText(frame, "Mouth Opened: {}".format(MOUTHTOTAL), (10, 330),
		cv2.FONT_HERSHEY_TRIPLEX, 0.7, (0, 204, 0), 2)

		# draw the computed mouth aspect ratio for the frame
		cv2.putText(frame, "mouth aspect ratio: {:.6f}".format(mouthMAR), (10, 360),
		cv2.FONT_HERSHEY_TRIPLEX, 0.7, (0, 204, 0), 2)

		# draw the looking direction for the frame
		cv2.putText(frame, "Looking direction: {}".format(lookingDirection), (10, 390),
		cv2.FONT_HERSHEY_TRIPLEX, 0.7, (0, 204, 0), 2)
	
	 	# display counter for how long the eyes are closed
		cv2.putText(frame, "Closed Eyes: {:.6f} SECONDS".format(SECONDS), (10, 420),
        cv2.FONT_HERSHEY_TRIPLEX, 0.7, (0, 204, 0), 2)

	    # display counter for how long the eyes are closed
		cv2.putText(frame, "Eyes are: {}".format(EYES), (10, 450),
        cv2.FONT_HERSHEY_TRIPLEX, 0.7, (0, 204, 0), 2)
