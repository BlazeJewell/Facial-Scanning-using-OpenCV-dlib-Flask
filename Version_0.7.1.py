...
def extract_eye(image, left, bottom_left, bottom_right, right, upper_right, upper_left):
	lower_bound = max([left[1], right[1], bottom_left[1],
                    bottom_right[1], upper_left[1], upper_right[1]])
	upper_bound = min([left[1], right[1], upper_left[1],
                    upper_right[1], bottom_left[1], bottom_right[1]])
	...
	cv2.line(image, (int((bottom_left[0] + bottom_right[0]) / 2), lower_bound),
	         (int((upper_left[0] + upper_right[0]) / 2), upper_bound), (0, 0, 255), 1)
...
table = PrettyTable()
table.field_names = ['Time', 'Eye Aspect Ratio', 'Right Eye Aspect Ratio', 'Left Eye Aspect Ratio',
                     'Blinks', 'Right Eye Blinks', 'Left Eye Blinks', 'Mouth Aspect Ratio', 'Mouth Opening Count']
...
while True:
	...
	for rect in rects:
		...
		# and draw them on the image
		count = 1
		right_eye = imutils.resize(extract_eye(
			frame, shape[36], shape[41], shape[40], shape[39], shape[38], shape[37]), width=100, height=50)
		...
		cv2.putText(frame, "Closed Eyes: {} SECONDS".format(SECONDS), (10, 330),
                    cv2.FONT_HERSHEY_TRIPLEX, 0.7, (0, 204, 0), 2)

		table.add_row([time.strftime("%a, %d %b %Y %H:%M:%S"), "{:.6f}".format(ear), "{:.6f}".format(rightEAR), "{:.6f}".format(
			leftEAR), "{}".format(TOTAL), "{}".format(RIGHTTOTAL), "{}".format(LEFTTOTAL),   "{:.6f}".format(mouthMAR),  "{}".format(MOUTHTOTAL)])
